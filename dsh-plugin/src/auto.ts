 ...
 * 这样 HMR 改 `config` 不需要重启进程。
 * @param ctx - 插件上下文（监听器随它的生命周期自动注销）。
 * @param config - 插件配置。
 */
export function applyAutoCapture(ctx: Context, config: Config): void {
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    ...
    if (!config.autoAfterTools.enabled) return decision
    if (!matchesToolName(config.autoAfterTools.matcher, exec.name)) return decision
    ...
    try {
      ...
      const value = await captureValue(ctx, config, config.autoAfterTools.delayMs, exec.signal)
  ...
  ctx.on('agent/turn-stopping', async ({ agent, turn, signal }) => {
    if (!config.autoOnTurnStop.enabled) return
    ...
    try {
      ...
      const value = await captureValue(ctx, config, config.autoOnTurnStop.delayMs, signal)
      ...
      if (config.autoOnTurnStop.steer) agent.steer(message)
